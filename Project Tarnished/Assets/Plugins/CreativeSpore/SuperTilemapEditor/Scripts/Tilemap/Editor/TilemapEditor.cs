    ...
    public partial class TilemapEditor : Editor
    {
        ...
        private void DoMapSceneGUI()
        {
            if (m_toggleMapBoundsEdit)
            {
                ...
                var fmh_1042_178_638319494674141999 = Quaternion.identity; Vector3 vMinX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MinGridX * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1043_185_638319494674163637 = Quaternion.identity; Vector3 vMaxX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2((m_tilemap.MaxGridX + 1f) * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1044_178_638319494674167025 = Quaternion.identity; Vector3 vMinY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, m_tilemap.MinGridY * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1045_185_638319494674170785 = Quaternion.identity; Vector3 vMaxY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, (m_tilemap.MaxGridY + 1f) * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
            ...
            if (EditorWindow.mouseOverWindow == SceneView.currentDrawingSceneView)
            {
                ...
                if (tile != null)
                {
                    ...
                    for (int i = 0; i < handlesPos.Count; ++i)
                    {
                        ...
                        {
                            var fmh_1143_75_638319494674177197 = Quaternion.identity; handlePos = Handles.FreeMoveHandle(handlePos, 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
